# -*- coding: utf-8 -*-
"""
Created on Tue May  7 17:03:09 2019
...
def sensitivity_function() :
      ...
      for x_val in variable_values:
            ...
            # fr = 0.25       # fraction of modified airway units

            for k in aw_ind:
                  if (regional_or_distributed == 'regional'):
                  # either grouped (regional modification) 
                        if k < fr * N :
                              var[k] = x_val
                        if k > (1. - fr) * N:
                              var[k] = 1. 
                      #       phi[k] = 1.+ (1. - mod_parameter)

                  # or distributed (local modification)
                  elif (regional_or_distributed == 'distributed'):     
                        if np.mod(k, np.int(1./fr)) == 0:
                              var[k] *= x_val
            ...
            # decide whiih variable to modify
            if variable_name == 'xi':
                  xi = var
            ...
            os.chdir(exe_path)
            subprocess.call(['Fractal.Lung.exe'])
            print '--> Processs Finished for : ', variable_name, x_val

            # load and unpack data
            print '--> Loading Data for : ', variable_name, x_val
            ...
            print '--> Copying Primary Results :', variable_name, x_val
            shutil.copyfile(data_path + '/primary_results', data_path + '/primary_results_' + str(variable_name) + '_' + str(x_val) + '.txt')           
      ...
      plt.savefig(save_path + '/sensitivity_results_' + str(variable_name) + '.pdf')
...
regional_or_distributed = int(sys.argv[2])
print 'regional or distributed : ', regional_or_distributed

fr = int(sys.argv[3])
print 'modification fraction : ', fr

variable_name = sys.argv[4]
print 'variable name : ', variable_name

variable_values = sys.argv[5:]
